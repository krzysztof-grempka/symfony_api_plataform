...
final class ResetPasswordMessage implements NotificationInterface
{
    ...
    public function notify(User $user, array $channels): void
    {
        ...
        $subject = $this->translator->trans('password.reset.subject', [], 'email');
        $body = $this->translator->trans('password.reset.body', ['%url%' => $this->resetPasswordUrl.'?token='.$token], 'email');
