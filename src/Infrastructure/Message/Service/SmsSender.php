...
class SmsSender implements SmsSenderInterface
{
    ...
    public function __construct(
        ...
        $this->service = (new SmsapiHttpClient())->smsapiPlService($this->apiToken);
    }
    ...
    {
        // @todo enable SMS send
        $smssBag = SendSmssBag::withMessage($recipients, $this->replaceAccents($body));
        $smssBag->test = $this->isTest;
        $this->service
           ->smsFeature()
           ->sendSmss($smssBag);
        // $senderEmail = is_string($this->parameterBag->get('emailAddress')) ? $this->parameterBag->get('emailAddress') : '';
        // $this->emailSender->sendBatch('SMS do: '.implode(' ', $recipients), $this->replaceAccents($body), $senderEmail, $senderEmail);
